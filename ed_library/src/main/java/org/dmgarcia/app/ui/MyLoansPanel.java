...
public class MyLoansPanel extends JPanel {
    ...
    private final JComboBox<String> cbFilterBy;
    ...
    public MyLoansPanel() {
        ...
        cbFilterBy = new JComboBox<>(new String[]{"Sin filtro", "Por libro"});
        ...
        loanModel = new DefaultTableModel(
                new Object[]{"ID", "Libro", "Fecha préstamo", "Fecha devolución", "Devuelto"}, 0
        ) {
            @Override
    ...
    private void loadData() {
        ...
        if ("Pendientes".equals(status)) {
            loans = loanRepo.findActive(Params.getUser());
        } else if ("Entregados".equals(status)) {
            loans = loanRepo.findReturned(Params.getUser());
        } else {
            loans = loanRepo.findAll(Params.getUser());
        }

        if (!"Sin filtro".equals(filterBy) && !value.isEmpty()) {
            if ("Por libro".equals(filterBy)) {
